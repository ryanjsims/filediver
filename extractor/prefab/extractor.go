...
func (c *unitContext) File() *stingray.File      { return c.file }
func (c *unitContext) Runner() *exec.Runner      { return c.ctx.Runner() }
func (c *unitContext) Config() map[string]string { return c.config }
func (c *unitContext) GetResource(name, typ stingray.Hash) (file *stingray.File, exists bool) {
	return c.ctx.GetResource(name, typ)
}
...
func ConvertOpts(ctx extractor.Context, unitCfg map[string]string, gltfDoc *gltf.Document) error {
	...
	for _, object := range prefabData.Objects {
		...
		if unitMetadata, contains := extras[unit.ID().Name.String()]; !contains {
			// We have not already loaded this unit, load it now
			unitCtx := newUnitCtx(ctx, unit, unitCfg)
			err := extr_unit.Convert(doc)(unitCtx)
			if err != nil {
				return err
			}
			extras, ok := doc.Extras.(map[string]map[string]interface{})
			...
			unitMetadata, ok := extras[unitCtx.File().ID().Name.String()]
			if !ok {
				return fmt.Errorf("could not resolve %s.unit gltf metadata? (this should not happen)", object.UnitHash.String())
			}
...
func Convert(currDoc *gltf.Document, unitCfg map[string]string) func(ctx extractor.Context) error {
	return func(ctx extractor.Context) error {
		return ConvertOpts(ctx, unitCfg, currDoc)
	}
}
